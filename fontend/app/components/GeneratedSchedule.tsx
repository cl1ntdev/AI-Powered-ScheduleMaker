...
const GeneratedSchedule = ({ result }: GeneratedScheduleProps) => {
  ...
  const days = Object.keys(editableSchedule);

  const handleUpdateTask = (day: string, taskIdx: number, field: string, value: string) => {
    const newSchedule = { ...editableSchedule };
    newSchedule[day][taskIdx][field] = value;
    ...
    <div className="w-full space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 pt-2">
        {days.map((day) => {
          ...
          return (
            <div key={day} className="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden flex flex-col">
              <div className="bg-gradient-to-r from-slate-900 to-slate-800 px-5 py-4">
                <h3 className="text-white font-bold text-sm uppercase tracking-[0.2em]">{day}</h3>
              </div>
              ...
              <div className="p-5 space-y-6 relative flex-1">
                ...
                {tasks.map((task: any, idx: number) => (
                  <div key={idx} className="flex gap-4 relative z-10 group/task items-start">
                    ...
                    <div className="flex-1 bg-slate-50 rounded-2xl p-3 border border-slate-100 focus-within:bg-white focus-within:border-indigo-300 transition-colors">
                      <div
                        ...
                        onBlur={(e) => handleUpdateTask(day, idx, "title", e.currentTarget.textContent || "")}
                      >
                        {task.title}
                      </div>
                    </div>
